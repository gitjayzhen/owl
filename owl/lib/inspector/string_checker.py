...
def AssertStrListContains_x_times(list_strs, str_key, count=1):
    """
    @param list_strs:需要比较的字符串列表
    @type list_strs:list
    @param str_key:标准字符串
    @type str_key:str
    @param count:包含次数
    ...
    """
    if type(str_key) == type(""):
        str_key = str(str_key)
    int_count = int(count)
    # print count
    for each in list_strs:
        if type(each) == type(str_key) and type(str_key) == type('str'):
            real_count = each.count(str_key)
            if real_count == int_count:
                continue
            else:
                raise CheckException("item1 contain item2 %d times, not %d times" % (real_count, int_count))
        else:
            raise CheckException("type %s and type %s should be str" % (type(each), type(str_key)))


def AssertNoNullItem(list_objs):
    """
    @param list_objs:比较对象
    @type list_objs: list
...
def AssertFileContains(str_file_name, str_key):
    """
    @param str_file_name:文件名
    @type str_file_name:str
    @param str_key:关键词
    @type str_key:str
    ...
    # print strFileName
    fp = open(str_file_name)
    try:
        for line in fp:
            if str_key.strip() in line.strip():
                ret = True
    finally:
        fp.close()

    if not ret:
        raise CheckException("%s can not found in file %s" % (str_key, str_file_name))
