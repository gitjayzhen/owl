...
def Long_Press(devices, element=None, x1=None, y1=None):
    try:
        ...
        x2 = int(x1) + 1
        y2 = int(y1) + 1
        click_cmd = 'adb -s %s shell input swipe %s %s %s %s %s' % (
        devices, str(x1), str(y1), str(x2), str(y2), str(2500))
...
def Swipe(devices, ele1=None, ele2=None, x1=None, y1=None, x2=None, y2=None, duration=" ", width=None, high=None):
    try:
        ...
        swipe_cmd = 'adb -s %s shell input swipe %s %s %s %s %s' % (
        devices, str(x1), str(y1), str(x2), str(y2), str(duration))
...
def Get_CrashByDropBox(devices, package):
    try:
        ...
        popen = subprocess.Popen(get_crashbydropbox_time_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                 shell=True)
        for i in range(100):
            line = popen.stdout.readline().strip('\r\n')
            if line == '': break
...
##    print(Get_Traffic(Get_DeviceId()[0],"com.**.activity.src")
##    print(Get_CpuMem(Get_DeviceId()[0],"com.**.activity.src")
##    Get_CrashByDropBox(Get_DeviceId()[0],"com.**.activity.src")
##    print(Get_Resolution(Get_DeviceId()[0])
##    Get_PhoneInfo(Get_DeviceId()[0])
##    Start_Activity(Get_DeviceId()[0],"com.**.activity.src/com.**.search.entry.EntryActivity")
##    Stop_Package(Get_DeviceId()[0],"com.**.activity.src")
##    Get_Battery_Level(Get_DeviceId()[0])
##    Get_Third_AppList(Get_DeviceId()[0])
##    Get_Start_TotalTime(Get_DeviceId(),"com.**.activity.src/com.**.search.entry.EntryActivity")
##    Install_App(Get_DeviceId()[0],"**Search_Debug.apk")
##    Uninstall_App(Get_DeviceId()[0],"com.**.activity.src")
##    Clear_App_Data(Get_DeviceId()[0],"com.**.activity.src")
...
##    Get_Start_TotalTime(Get_DeviceId(),"com.**.activity.src/com.**.search.entry.EntryActivity")
##    Is_install(Get_DeviceId()[0],"com.**.activity.src")
