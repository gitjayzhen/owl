...
from com.framework.core.adb.AndroidDebugBridge import AndroidDebugBridge
...
class AppiumBaseApi(object):
    ...
    def find_element_by_want(self, by, value, timeout=3):
        """
        通过元素名称定位单个元素
    ...
    def sendkey_by_h5(self, by, value, txt):
        """
        根据webview页面标签的属性来进行内容输入
        :param by:
        :param value:
        :param txt:
        :return:
        """
        elemt = self.find_element_by_want(by, value)
        tag_name = elemt.tag_name
        if tag_name == 'input':
            input_type = elemt.get_attribute('type')
            if input_type == 'text' or 'password':
                self.do_sendkeys(by, value, txt)
            elif input_type == 'radio':
                self.do_select_radio(by, value, txt)
            elif input_type == 'checkbox':
                self.do_select_checkbox(by, value, txt)
        elif tag_name == 'select':
            self.do_select_option(by, value, txt)
        elif tag_name == 'textarea':
            self.do_sendkeys(by, value, txt)
        elif tag_name == 'file':
            # 格式化这个文件路径
            txt = txt.replace("\\", os.path.sep)
            txt = txt.replace("/", os.path.sep)
            # 检查是否是绝对路径
            if not os.path.exists(txt):
                txt = os.path.join(os.getcwd(), txt)
